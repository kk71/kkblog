...
#=======================================================
def common_view(navbar_active):
	'''
for public view rendering(titlebar and sidebar)

return:
	dict()
'''
	freqs=[]
	for i in models.list_articles(items_per_result=6,sort_by_frequence=True):
		freqs.append(models.article_obj_to_dict(i))
	return {
			"blog_author":settings.author_name,
			"blog_description":settings.blog_description,
			"weibo":settings.weibo,
			"twitter":settings.twitter,
			"self_icon":settings.page_icon,
			"brief_selfintro":settings.brief_selfintro,
			"weibo_box":settings.weibo_box,
			"twitter_follow":settings.twitter_follow,
			"email_addr":settings.email,
			"title":"%s的博客 - %s"%(settings.author_name,settings.brand),
			"navbar_active":navbar_active,
			"category_list":models.category.objects.all(),
			"hot_list":freqs,
			"tag_list":models.list_tags(),
			"music_list":settings.bgmusic,
			"friend_list":settings.friend_list,
...
'''
	...
	if len(models.article.objects.all())%settings.articles_per_page!=0:
		pages=len(models.article.objects.all())//settings.articles_per_page+1
	else:
...
'''
	...
	if offsets==("","") or offsets==("",) or offsets==():
		...
		i=models.archive.objects.get(year=int(offsets[0]),month=int(offsets[1]))
		atcls=[]
...
'''
	...
	if offsets==("","") or offsets==("",) or offsets==():
		...
		tags_atcls=models.list_tags_with_articles()#items are tagname:articles_list
		for i in tags_atcls:
		...
		try:
			article_objs=models.tag.objects.get(name=offsets[0]).articles.order_by("-datetime")
			ar=[]
...
'''
	try:
		...
		if atcle.shown==False:raise Exception
...
'''
	...
	ar=[]
	if offsets==("","") or offsets==("",) or offsets==():
		c=models.category.objects.annotate(article_count=Count("articles")).order_by("-article_count")#sort by the count of articles combined to the category
		for i in c:
...
'''
	try:
		...
		return render_to_response("404.html",status=404)
	cmt_list=models.list_comments(isolated=False,article_id=atcle_id,comment_per_result=999)
	dic={
			"comment_list":cmt_list,
			"pagi":False,
	}	
